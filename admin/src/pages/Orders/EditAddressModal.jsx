...
const EditAddressModal = ({ open, handleClose, currentAddress, handleSave }) => {
  ...
  useEffect(() => {
    // Gán lại địa chỉ mỗi khi `currentAddress` thay đổi
  ...
  useEffect(() => {
    const fetchCities = async () => {
      try {
        ...
        // Tìm city hiện tại từ currentAddress
        const currentCity = response.data.find((c) => c.name === currentAddress.city);
        if (currentCity) {
          setCity(currentCity); // Set giá trị thành phố hiện tại
          fetchDistricts(currentCity.code); // Tự động fetch districts dựa trên city hiện tại
        }
      } catch (error) {
    ...
    const fetchDistricts = async (cityCode) => {
      try {
        ...
        // Tìm district hiện tại từ currentAddress
        const currentDistrict = response.data.districts.find((d) => d.name === currentAddress.district);
        if (currentDistrict) {
          setDistrict(currentDistrict); // Set giá trị quận/huyện hiện tại
          fetchWards(currentDistrict.code); // Tự động fetch wards dựa trên district hiện tại
        }
      } catch (error) {
    ...
    const fetchWards = async (districtCode) => {
      try {
        ...
        // Tìm ward hiện tại từ currentAddress
        const currentWard = response.data.wards.find((w) => w.name === currentAddress.ward);
        if (currentWard) {
          setWard(currentWard); // Set giá trị phường/xã hiện tại
        }
      } catch (error) {
