  ...
  try {
    ...
    const monthlySales = await OrderItem.findAll({
      attributes: [
        ...
        [sequelize.literal('SUM(OrderItem.price * OrderItem.quantity)'), 'totalSales'],
        [sequelize.fn('SUM', sequelize.col('OrderItem.quantity')), 'totalProductsSold']
      ],
      include: [{
        model: Order,
        attributes: [],
...
exports.getRevenueByProduct = async (req, res) => {
  try {
    const revenueByProduct = await OrderItem.findAll({
      attributes: [
        'product_id',
        [sequelize.literal('SUM(OrderItem.price * OrderItem.quantity)'), 'totalRevenue'] // Chỉ định rõ OrderItem.price và OrderItem.quantity
      ],
      group: ['product_id'],
      include: [{
        model: Product,
  ...
  try {
    ...
    const actualRevenue = await OrderItem.findAll({
      attributes: [
        ...
        [sequelize.fn('SUM', sequelize.literal('OrderItem.price * OrderItem.quantity')), 'totalActualRevenue']  // Tính tổng doanh thu thực tế (OrderItem.price * OrderItem.quantity)
      ],
      where: {
